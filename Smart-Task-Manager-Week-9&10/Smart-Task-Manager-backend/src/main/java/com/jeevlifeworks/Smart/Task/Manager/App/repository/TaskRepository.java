...
public interface TaskRepository extends JpaRepository<Task, Long>{
	...
	/**
	 * Counts the number of tasks per status for a specific user.
	 * 
	 * Useful for analytics and dashboard summaries.
	 *
	 * Accepts parameter userId ID of the user
	...
	@Query("SELECT t.status as status, COUNT(t) as count FROM Task t " +
			"JOIN t.assignees a " +
			"WHERE a.id = :userId GROUP BY t.status")
	List<Object[]> countTasksByStatusForUser(@Param("userId") Long userId);


	/**
     * Counts how many tasks each employee has completed.
     * 
     * return A list of Object arrays where:
     *         [0] = username (String)
     *         [1] = completed task count (Long)
     */
	@Query("SELECT u.username, COUNT(t) as compCount FROM Task t " +
			"JOIN t.assignees u " +
			"WHERE t.status = 'Completed' GROUP BY u.username")
	List<Object[]> completedTasksPerEmployee();

	/**
     * Finds both upcoming and overdue tasks for a specific user.
     * 
     * Orders them by due date in ascending order.
     *
     * Accepts parameter userId ID of the user
     * return List of tasks (upcoming and overdue)
     */
	@Query("SELECT t FROM Task t JOIN t.assignees a " +
			"WHERE a.id = :userId AND (t.dueDate >= CURRENT_DATE OR t.dueDate < CURRENT_DATE) " +
			"ORDER BY t.dueDate ASC")
	List<Task> findUpcomingAndOverdue(@Param("userId") Long userId);
